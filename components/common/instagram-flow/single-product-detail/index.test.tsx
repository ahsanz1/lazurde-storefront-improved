...
import SingleProductDetail from "./index";
...
test("testing insta post component", async () => {
  await waitFor(() => {
    render(
      <QueryClientProvider client={queryClient}>
        <ContextProvider>
          <SingleProductDetail
            ...
            priceListId="1000"
            setNotifyModalOpen={jest.fn()}
          />
        </ContextProvider>
      </QueryClientProvider>
    );
    const wishlist = screen.getByRole("wishlist");
    ...
    // const addtocart = screen.queryByRole("addtocart");
    // expect(addtocart).not.toBeInTheDocument();

    const notifyMemodalBtn = screen.getByTestId("notifyMemodalBtn");
    expect(notifyMemodalBtn).toBeInTheDocument();
    fireEvent.click(notifyMemodalBtn);
  });
});
